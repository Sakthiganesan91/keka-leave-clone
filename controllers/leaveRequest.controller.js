...
function getLeaveDaysByMonth(startDateTime, endDateTime) {
  ...
  for (
    ...
    leaveDaysByMonth[key] = (leaveDaysByMonth[key] || 0) + leaveHours;

    d = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1); // reset time to midnight
  }
...
const validateLeaveCount = async (
  ...
  for (const year of yearArray) {
    const [employee_leave_yearly] = await connection.query(
      `SELECT leave_taken,rollover_prev_year FROM employee_leave_yearly 
    ...
    const rolloverCount = employee_leave_yearly[0]?.rollover_prev_year ?? 0;

    flag = !(leaveCount < leavePolicy.max_days_per_year + rolloverCount);
    if (flag) {
...
const updateAttendance = async (
  ...
  const TIME_VALUES = {
    FULL_DAY_START: "09:00:00",
    FULL_DAY_END: "18:00:00",
    HALF_DAY_MORNING_END: "13:00:00",
    HALF_DAY_AFTERNOON_START: "14:00:00",
  };
  ...
  for (let date = new Date(start); date <= end; ) {
    ...
    if (
      ...
      if (startHour === 9 && endHour === 13) {
        isHalfDay = 1;
        halfDayType = "first half";
        startTime = TIME_VALUES.FULL_DAY_START;
        endTime = TIME_VALUES.HALF_DAY_MORNING_END;
      } else if (startHour === 14 && endHour === 18) {
        isHalfDay = 1;
        halfDayType = "second half";
        startTime = TIME_VALUES.HALF_DAY_AFTERNOON_START;
        endTime = TIME_VALUES.FULL_DAY_END;
      }
    ...
    await connection.query(
      `INSERT INTO attendance (
        employee_id, email, date, start_time, end_time,
        is_present, is_leave, leave_type, leave_id,
...
const updateEmployeeLeave = async (
  ...
  for (const [year, days] of Object.entries(leaveDaysByMonth)) {
    ...
    const [rolloverPrevYearCount] = await connection.query(
      `SELECT rollover_prev_year FROM employee_leave_yearly 
     WHERE employee_id = ? AND leavepolicy_id = ? AND year = ?`,
      [employeeId, leavePolicyId, yr]
    );

    const leaveTaken = totalTakenResult[0].total || 0;
    const rollOverLeave = rolloverPrevYearCount[0].rollover_prev_year ?? 0;
    const remaining = max_days_per_year + rollOverLeave - leaveTaken;
